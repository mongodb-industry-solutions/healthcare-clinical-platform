...
export function PatientComparison() {
  // Default to Maria (target/high-risk) vs James (healthy) - the demo scenario
  const [leftPatientId, setLeftPatientId] = React.useState("pt-001-maria-garcia")
  const [rightPatientId, setRightPatientId] = React.useState("pt-002-james-wilson")

  const leftPatient = mockPatients.find((p) => p.patient_id === leftPatientId)!
  const rightPatient = mockPatients.find((p) => p.patient_id === rightPatientId)!
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <Alert className="bg-primary/5 border-primary/20">
        ...
        <AlertDescription className="text-muted-foreground">
          Maria and James have nearly identical vital signs, but the system responds completely differently 
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            How clinical context changes the system response
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <DifferenceCard
              ...
              left={{
                ...
                note: leftPatient.active_alerts.length > 0 
                  ? leftPatient.active_alerts[0].severity.toUpperCase()
                  : "No alerts",
              }}
              right={{
                label: rightPatient.demographics.given,
                value: `${rightPatient.active_alerts.length} active alerts`,
                note: rightPatient.active_alerts.length > 0 
                  ? rightPatient.active_alerts[0].severity.toUpperCase()
                  : "No alerts",
              }}
            />
            <DifferenceCard
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2 text-sm">
              <li className="flex items-start gap-2">
                ...
                <span>
                  <strong>A heart rate of 95 bpm</strong> in a patient on beta-blockers is highly concerning 
                  because the medication should be suppressing heart rate to 55-75 bpm.
                </span>
              </li>
              <li className="flex items-start gap-2">
                <ArrowRight className="h-4 w-4 text-primary shrink-0 mt-0.5" />
...
function PatientSelector({
  ...
  const availablePatients = mockPatients.filter((p) => p.patient_id !== excludeId)
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {patient.demographics.age}y | {patient.profile_type}
                  </div>
                </div>
              </div>
            </DropdownMenuItem>
...
function ComparisonCard({ patient }: { patient: Patient360 }) {
  ...
  return (
    <Card className={cn(
      ...
      hasHigh && !hasCritical && "border-warning/50"
    )}>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>
                {patient.demographics.age}y {patient.demographics.gender === "female" ? "F" : "M"} | {patient.hospital_name.split(" ")[0]}
              </CardDescription>
            </div>
          </div>
...
function getMedShortName(display: string): string {
